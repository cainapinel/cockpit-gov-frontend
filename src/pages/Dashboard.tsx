        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Activity className={`h-4 w-4 ${
              data.sentiment_analysis === 'Positivo' ? 'text-emerald-500' :
              data.sentiment_analysis === 'Crítico' ? 'text-destructive' :
              'text-amber-500'
            }`} />
          </CardHeader>
          <CardContent>
            <div className={`text-2xl font-bold ${
              ...
              data.sentiment_analysis === 'Crítico' ? 'text-destructive' :
              'text-amber-500'
            }`}>{data.sentiment_analysis}</div>
            <p className="text-xs text-muted-foreground">
              {data.sentiment_trend || 'OSINT + IA (cache 2h)'}
            </p>
          </CardContent>
        </Card>
      </div>
